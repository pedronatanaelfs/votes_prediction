...
def perform_clustering(vectors, n_clusters=None, max_clusters=20):
    ...
    # Hierarchical clustering with detailed progress prints
    logger.info(f"Performing hierarchical clustering with {n_clusters} clusters...")
    print(f"[Hierarchical] Starting AgglomerativeClustering with n_clusters={n_clusters}")
    print(f"[Hierarchical] Preparing AgglomerativeClustering model...")
    hierarchical = AgglomerativeClustering(n_clusters=n_clusters)
    print(f"[Hierarchical] Model initialized: {hierarchical}")
    print(f"[Hierarchical] Fitting model to vectors of shape {vectors.shape}...")
    hierarchical_labels = hierarchical.fit_predict(vectors)
    print(f"[Hierarchical] Fitting complete.")
    print(f"[Hierarchical] Number of clusters assigned: {n_clusters}")
    print(f"[Hierarchical] First 10 cluster assignments: {hierarchical_labels[:10]}")
    print(f"[Hierarchical] All cluster assignments: {hierarchical_labels}")
    unique, counts = np.unique(hierarchical_labels, return_counts=True)
    print(f"[Hierarchical] Cluster distribution:")
    for u, c in zip(unique, counts):
        print(f"    Cluster {u}: {c} items")
    clustering_results['hierarchical'] = hierarchical_labels
    print(f"[Hierarchical] Results stored in clustering_results['hierarchical']")
